...
def parse_headers(text):
    r = requests.get(form_url, header_text)
...
def main():
    ...
    url = form_url.replace(
        "/viewform", "/formResponse"
    )  # URL changes for submission ^_^
    ...
    while True:
        ...
        # every form has diffent entry ids
        values1 = {
        ...
        for x in range(form_count):
            for i in schedule[str(period)]:
                ...
                input("URL to be submitted: " + r.url)
                print("\nSubmitting form...\n")
                ...
                try:
                    ...
                    r = requests.post(url, data=form_values, headers=headers)
                    sleep(5)
                    clear()
